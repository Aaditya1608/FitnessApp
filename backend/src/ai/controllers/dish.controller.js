...
async function saveDish(req, res) {
  try {
    ...
    const checkQuery = `SELECT * FROM saved_dishes WHERE user_id = $1 AND title = $2`;
    const checkResult = await pool.query(checkQuery, [userId, title]);
    if (checkResult.rows.length > 0) {
      return ApiResponse(res, 200, "Dish already saved", checkResult.rows[0]);
    }
...
async function getLoggedDishes(req, res) {
  try {
    const userId = req.user.id;
    let query = `SELECT * FROM dish_history WHERE user_id = $1 ORDER BY logged_at DESC;`;
    const result = await pool.query(query, [userId]);
    return ApiResponse(res, 200, "Logged dishes fetched successfully", result.rows);
  } catch (err) {
    return ApiError(res, 500, "Server Error");
  }
}

async function getCustomDishes(req, res) {
  try {
    const userId = req.user.id;
    let query = `SELECT * FROM custom_dishes WHERE user_id = $1 ORDER BY created_at DESC;`;
    const result = await pool.query(query, [userId]);
    return ApiResponse(res, 200, "Custom dishes fetched successfully", result.rows);
  } catch (err) {
    return ApiError(res, 500, "Server Error");
  }
}
export default { saveDish, logDish, getSavedDishes, deleteSavedDish, getLoggedDishes, getCustomDishes };